...
bsGrade.addEventListener('change', function() {
    ...
    fetch(`/admin/api/grades/${branchId}`)
        ...
        .then(data => {
            ...
            populateSelect(bsStream, streams.map(s => ({ id: s, name: s })), 'All');
        });
...
bsBtn.addEventListener('click', function() {
    ...
    fetch(`/admin/api/broadsheet?branch_id=${branchId}&class_id=${classId}&exam_id=${examId}&stream=${stream || ''}`)
        .then(res => res.json())
        .then(data => {
            if (data.error) {
            ...
            // Only use this UI for grades other than Form 3/4
            // if (["Form 3", "Form 4"].includes(gradeName)) {
            //     bsContainer.innerHTML = `<p class="text-warning text-center">This broadsheet UI is only for CBC grades.</p>`;
            //     return;
            // }

            function gradeBadge(grade){
                if (!grade) return "";
                const map = { "EE":"success","ME":"primary","AE":"warning","BE":"danger" };
            ...
            function performanceBar(analysis){
                ...
                let bars = "";
                const colors = {"EE":"bg-success","ME":"bg-primary","AE":"bg-warning","BE":"bg-danger"};
            ...
            <div class="text-center mb-4">
                <h4 class="fw-bold text-center text-uppercase text-secondary">
                    <i class="bi bi-mortarboard-fill mx-2"></i>${data.branch_name}
                </h4>
                <h6 class="text-secondary">${data.exam_name} Broadsheet & Analytics</h6>
                <h6 class="fw-bold text-secondary text-uppercase">
                    ${gradeName} ${stream ? '- ' + stream : ''}</h6>
                <hr>
            </div>
            ...
            subjects.forEach(subj=>{
                ...
                let breakdown=Object.entries(analysis).map(([g,c])=>`${g} (${c})`).join(", ");
                html+=`
                    <div class="mb-3">
            ...
            if(atRisk.length){
                html+=`<div class="alert alert-danger mx-3">
                    ...
                    <ul class="mb-0 mt-2">`;
                atRisk.slice(0,2).forEach(s=>{ html+=`<li>${s.name} (${s.low_subjects})</li>`; });
                if(atRisk.length>2){
                    ...
                    atRisk.slice(2).forEach(s=>{ html+=`<li>${s.name} (${s.low_subjects})</li>`; });
                    html+=`</div>`;
                }
                html+=`</ul></div>`;
            }
            ...
            subjects.forEach(subj=>{ html+=`<th>${subj.name}<br><small class="text-warning">${subj.teacher}</small></th>`; });
            html+=`</tr></thead><tbody>`;
            ...
            students.forEach(student=>{
                ...
                subjects.forEach(subj=>{
                    ...
                    let display="-";
                    if(s && s.marks!="-") display=`${s.marks}<br>${gradeBadge(s.grade)}`;
                    html+=`<td>${display}</td>`;
                });
                html+=`</tr>`;
            });
            ...
            new Chart(ctx,{
                ...
                data:{
                    ...
                    datasets:[{label:'Mean Score', data:subjects.map(s=>averages[s.id]||0), backgroundColor:'rgba(54,162,235,0.7)', borderColor:'rgba(54,162,235,1)', borderWidth:1}]
                },
                options:{responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true, max:100, title:{display:true,text:'Score'}}, x:{title:{display:true,text:'Subjects'}}}}
            });
